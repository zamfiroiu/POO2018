...
void main() {
	...
	Locuinta *l=new SuperApartament();

	cout <<  l->getIntretinere()<<endl;
	l = &a;
	cout << l->getIntretinere() << endl;
	
	//SuperApartament s;
	//a = s;
	//cout << a.getIntretinere();
	Apartament *ap;
	ap = new Apartament();
	ap = new Apartament[3];
	...
	Locuinta **bp;
	bp = new Locuinta*[4];
	bp[0] = new Apartament("Adresa", 2, new float[2]{ 34,45 }, 2);
	bp[1] = new Apartament("Adresa 2", 3, new float[3]{ 34,45,23 }, 2);
	bp[2] = new SuperApartament(3);
	bp[3] = new SuperApartament(8);

	for (int i = 0; i < 4; i++) {
		cout<<bp[i]->getIntretinere();
	}
